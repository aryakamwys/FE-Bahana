import React from "react";

const ContentBlog = () => {
  return (
    <div className="lg:pl-14 py-[15px] lg:py-10">
      <div className="flex flex-col items-start">
        <div className="font-inter font-semibold text-black text-[14px] md:text-[16px] lg:text-[20px]">
          What is the issue?
        </div>
        <div className="h-[5] lg:h-[10px] "></div>
<<<<<<< HEAD
        <div className="font-inter font-semibold text-black text-start text-[10px] lg:text-[17px] lg:leading-normal md:text-[14px]  ">
          Lorem ipsum dolor sit amet consectetur. Mauris feugiat leo venenatis
          morbi quam viverra. Velit lectus rhoncus molestie gravida dolor sem
          commodo suspendisse pellentesque. Quam placerat vitae duis turpis
          adipiscing massa. Consequat elit quis congue scelerisque varius mi
          quam volutpat. Vitae eget sollicitudin nec ut. Enim elit sit tellus
          pharetra sed. Amet in felis ac fermentum. Quam cras sed nisl amet ut.
          Erat mi ante molestie et egestas massa nulla convallis. Mauris libero
          id sed libero vulputate duis eu odio aenean. Pulvinar vel sodales sed
          tortor. Non lectus amet est nisl feugiat.
        </div>
        <div className="font-inter font-semibold text-black text-start text-[10px] lg:text-[17px] lg:leading-normal md:text-[14px] mt-2  ">
          Tempus maecenas integer auctor in sollicitudin elementum lorem leo
          nisi. Egestas laoreet egestas egestas volutpat. Vulputate suspendisse
          tincidunt nibh habitant id. Turpis mi aliquet enim natoque risus
          ridiculus feugiat. Ut augue quam molestie pharetra massa sagittis. Vel
          ultrices posuere nullam sit ut. Ac turpis molestie tincidunt viverra
          pharetra bibendum. Accumsan magna at enim quis facilisi nisl dictumst.
          Lacus mattis quam turpis vitae nulla mauris nunc. Proin habitant
          tortor quis eros non eu. Tellus interdum dui vel eget fames tincidunt
          et. Et cras sapien id nec congue sed. Eget cras porttitor amet
          porttitor non turpis ullamcorper. Sed in turpis orci nunc semper.
          Tincidunt luctus varius praesent viverra congue consectetur commodo
          duis ipsum.
        </div>
        <div className="font-inter font-semibold text-black text-start text-[10px] lg:text-[17px] lg:leading-normal md:text-[14px]  mt-2">
          Nunc in urna blandit et. Condimentum nunc justo sagittis integer velit
          ut adipiscing nunc. Mauris nulla vivamus semper arcu diam est eget
          ultrices morbi. Tellus imperdiet vitae integer lorem mi. Congue diam
          odio habitasse nulla morbi eget pharetra auctor consectetur. Sem
          pretium non faucibus at. Amet massa ligula tortor morbi mauris non.
          Aliquam aliquam platea id imperdiet enim blandit. Proin in dui arcu
          sagittis sed sollicitudin sit orci quisque.A odio turpis neque aliquam
          natoque libero feugiat. Integer ac nulla id risus sagittis metus
          aliquam blandit. Habitant volutpat vitae interdum in at nunc.
        </div>
        <div className="font-inter font-semibold text-black text-start text-[10px] lg:text-[17px] lg:leading-normal md:text-[14px]  mt-2">
          Risus lectus magna penatibus a eu fermentum. Purus pharetra amet sit
          arcu consectetur faucibus quam. Ac habitant mauris dui mollis. Nulla
          odio vestibulum in parturient viverra et non risus turpis. Placerat
          sit nisi et aenean blandit ut sed fames cursus. Nunc posuere aliquam
          cras orci mattis sed aliquam amet tristique. Vitae sit iaculis vel in
          nisi lectus commodo accumsan. Dui donec at enim vitae sagittis sem
          purus. Dictum sagittis vestibulum gravida quis leo at. A vitae in
          tincidunt elementum eu a venenatis fringilla. Sed morbi mauris vitae
          nec. Lectus ultrices egestas risus posuere quam elementum pulvinar sit
          diam.
        </div>
        <div className="font-inter font-semibold text-black text-start text-[10px] lg:text-[17px] lg:leading-normal md:text-[14px]  mt-2">
          Venenatis congue morbi magna dolor at scelerisque. Eleifend adipiscing
          lacus laoreet vel amet netus. Diam quis imperdiet faucibus risus
          turpis commodo lacus id. Suspendisse dictum ultrices aenean ornare
          quisque id eu. Feugiat aenean cursus ipsum feugiat consequat fringilla
          urna. Scelerisque tempus viverra semper in morbi. Ornare tortor a id
          enim. Gravida proin id aenean sit lectus lacus vulputate condimentum.
          Leo in tellus elit diam. Enim adipiscing.
=======
        <div className="font-inter font-medium text-black text-start text-[10px] md:text-[12px] lg:text-[15px] lg:leading-normal">
          Musim El Nino merupakan fenomena alam yang ditandai oleh pemanasan
          suhu permukaan laut di wilayah Pasifik bagian tengah dan timur.
          Fenomena ini sering kali membawa dampak besar pada pola cuaca global,
          termasuk di Indonesia. Dampak tersebut tidak hanya dirasakan dalam
          bentuk perubahan curah hujan dan suhu, tetapi juga berdampak
          signifikan pada sektor pertanian.
        </div>
        <div className="font-inter font-medium text-black text-start py-2 text-[10px] md:text-[12px] lg:text-[15px] lg:leading-normal">
          Musim El Nino meyebabkan 3 dampak utama bagi pertanian yaitu,
          penurunan curah hujan, kenaikan suhu, dan ketidakseimbangan pola
          tanam. Tiga dampak ini mengakibatkan satu masalah yaitu kelangkaan
          produk pertanian. Dengan adanya kelangkaan produk mengakibatkan
          kenaikan harga pasar yang sangat tinggi sehingga merugikan konsumen.
          Beras, jagung, dan beberapa sayuran yang berperan penting dalam
          masakan seperti bawang dan cabai merupakan incaran bagi konsumen di
          Indonesia. Musim El Nino bisa dikatakan pengancam kesejahteraan
          masyarakat Indonesia. Konsumen akan mengalami krisis produk pertanian
          sedangkan petani akan rugi akibat pendapatan yang sedikit akibat
          produk yang dijual sangat mahal sehingga kestabilan ekonomi akan
          hancur.
        </div>
        <div className="font-inter font-medium text-black text-start py-2  text-[10px] md:text-[12px] lg:text-[15px] lg:leading-normal">
          Suatu permasalahan tentunya ada solusi. Langkah yang bisa dilakukan
          adalah diversifikasi tanaman. Diversifikasi tanaman yang dimaksud ini
          adalah dengan memilih variestas tanaman yang produksinya terjaga saat
          musim El Nino dan melakukan rotasi tanaman untuk peningkatan kesuburan
          tanaman. Langkah lainnya adalah dengan memberikan edukasi kepada
          petani terkait teknologi dan inovasi pertanian. Kita dapat memberikan
          informasi menganai teknologi konversi air serta teknologi real time
          untuk menjaga kesetabilan pertanian. Pemerintah juga dapat menjalin
          kerjasama internasional meliputi sharing session serta bantuan.
        </div>
        <div className="font-inter font-medium text-black text-start py-2  text-[10px] md:text-[12px] lg:text-[15px] lg:leading-normal">
          Musim El Nino adalah ancaman nyata bagi sektor pertanian dan harga
          pasar produk pertanian. Namun, dengan langkah-langkah yang tepat,
          dampaknya dapat diminimalkan. Penerapan langkah solusi yang tepat
          sesuai penjelasan diatas dapat mengatasi kerugian antar sesama pelaku
          ekonomi untuk menciptakan keseimbangan antar masyarakat Indonesia.
          Dengan kata lain, kita dapat menjaga stabilitas harga pasar dan
          kesejahteraan petani di tengah kondisi cuaca yang tidak menentu.
>>>>>>> 6c040d0aac418fc0e84f295c2bcc6047173383b0
        </div>
      </div>
    </div>
  );
};

export default ContentBlog;
